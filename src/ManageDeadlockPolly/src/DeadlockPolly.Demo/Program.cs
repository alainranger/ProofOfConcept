...
using DeadlockPolly.Core.Repositories;
using DeadlockPolly.Core.RetryPolicies;
using Microsoft.Data.SqlClient;

namespace DeadlockPolly.Demo;

internal static class Program
{
    private const string DefaultConnectionString =
        "Server=localhost,1433;Database=DeadlockTestDb;User Id=sa;Password=YourStrong!Pass2024;" +
    ...
    private static async Task EnsureDatabaseReadyAsync(string connectionString)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            catch
            {
                ...
                {
                    throw new InvalidOperationException("SQL Server n'a pas répondu après 30 tentatives");
                }
            }
        }
    }
    ...
    private static async Task InitializeDatabaseAsync(string connectionString)
    {
        ...
        const string createSchemaSql = """
            ...
            BEGIN
                ...
                INSERT INTO dbo.DeadlockTest (Id, Value) VALUES (1, 0), (2, 0);
                CREATE NONCLUSTERED INDEX IX_DeadlockTest_Value ON dbo.DeadlockTest (Value);
            END
            """;
        ...
        await using (var masterConn = new SqlConnection(builder.ConnectionString))
        {
            ...
            await using var cmd = new SqlCommand(createDatabaseSql, masterConn);
        ...
        await using (var appConn = new SqlConnection(connectionString))
        {
            ...
            await using var cmd = new SqlCommand(createSchemaSql, appConn);
            await cmd.ExecuteNonQueryAsync();
        }
